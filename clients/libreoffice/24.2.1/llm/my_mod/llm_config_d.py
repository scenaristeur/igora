...
if TYPE_CHECKING:
    ...
    from ooodev.dialog.dl_control.ctl_input_field import CtlInputField
...
class LlmConfigD:
    def __init__(self, text, font, config):
        ...
        self._margin = 6
        self._box_height = 30
        self._title = "Llm Config"
        self._msg = "config your llm settings"
        ...
        self._system_prompt: CtlInputField | None = None
    ...
    def _init_handlers(self) -> None:
        ...
        self._fn_on_system_prompt_changed = self.on_system_prompt_changed
    ...
    def _init_system_prompt(self) -> None:
        """Add a text field to the dialog control"""
        sz_lbl = self._ctl_main_lbl.view.getPosSize()
        self._ctl_sys_prompt = Dialogs.insert_text_field(
            dialog_ctrl=self._dialog.control,
            x=self._margin,
    ...
    #         label="Group Box TWO",
    #     )
    #     self._ctl_gb2.set_font_descriptor(
    #         self._ctl_gb1.font_descriptor.component)
    ...
    def _init_buttons(self) -> None:
        ...
        self._ctl_btn_info.add_event_action_performed(
            self._fn_button_action_preformed)
    ...
    def _handle_results(self, result: int) -> None:
        ...
        if result == MessageBoxResultsEnum.OK.value:
            #msg = ""
            # msg = str(self._ctl_sys_prompt)
            msg = self._ctl_sys_prompt.text
            # if self._system_prompt:
            #     msg += f"System Prompt: {self._system_prompt}\n"
    ...
    def on_system_prompt_changed(
        self, src: Any, event: EventArgs, control_src: CtlInputField, *args, **kwargs
    )   -> None:
        self._system_prompt = control_src.text
        prompt = str(control_src.text)
        print(prompt)
    ...
    def show(self) -> int:
        ...
        self._dialog.set_pos_size(
            x, y, self._width, self._height, PosSize.POSSIZE)
